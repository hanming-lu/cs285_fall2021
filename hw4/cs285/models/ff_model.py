...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        obs_normalized = normalize(obs_unnormalized, obs_mean, obs_std) # DONE(Q1)
        acs_normalized = normalize(acs_unnormalized, acs_mean, acs_std) # DONE(Q1)
        ...
        delta_pred_normalized = self.delta_network(concatenated_input) # DONE(Q1)
        next_obs_pred = obs_unnormalized + unnormalize(ptu.to_numpy(delta_pred_normalized), delta_mean, delta_std) # DONE(Q1)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        prediction, _ = self(obs, acs, **data_statistics) # DONE(Q1) get numpy array of the predicted next-states (s_t+1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        target = normalize(next_observations - observations, data_statistics['delta_mean'], data_statistics['delta_std']) # DONE(Q1) compute the normalized target for the model.
        ...
        _, pred_norm = self(observations, actions, **data_statistics)
        loss = self.loss(pred_norm, target) # DONE(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.

...
class MPCPolicy(BasePolicy):
        ...
        if self.sample_strategy == 'random' \
            ...
            for i in range(self.cem_iterations):
                ...
                if i == 0:
                  ...
                  candidate_action_sequences = np.random.normal(elite_mean, elite_std, size=(num_sequences, horizon, self.ac_dim))

                # - Get the top `self.cem_num_elites` elites
                #     (Hint: what existing function can we use to compute rewards for
                #      our candidate sequences in order to rank them?)
                predicted_rewards = self.evaluate_candidate_sequences(candidate_action_sequences, obs)
                idx_top = np.argpartition(predicted_rewards, -self.cem_num_elites)
                elite_action_sequences = candidate_action_sequences[idx_top][-self.cem_num_elites:]
    ...
    def evaluate_candidate_sequences(self, candidate_action_sequences, obs):
        ...
        pred_sum_of_rewards = np.zeros(N)
        
        # for each model in ensemble
        for model in self.dyn_models:
          # compute the predicted sum of rewards for each candidate action sequence.
          pred_sum_of_rewards += self.calculate_sum_of_rewards(obs, candidate_action_sequences, model)

        # return the mean predictions across all ensembles
        mean_pred = pred_sum_of_rewards / len(self.dyn_models)
        return mean_pred
    ...
    def calculate_sum_of_rewards(self, obs, candidate_action_sequences, model):
        ...
        #       action sequence.
        obs = np.tile(obs, (N, 1))
        for i in range(self.horizon): #sum across `self.horizon` time step
            acs = candidate_action_sequences[:, i, :]
            rewards, _ = self.env.get_reward(obs, acs)
